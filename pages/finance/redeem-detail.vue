    ...
    <view class="order-list">
      <view v-for="(order, index) in orders" :key="index" class="order-card" @tap="viewOrderDetail(order)">
        <view class="order-header">
          <view class="left">
            <text class="order-no">赎货单号{{ order.sn }}</text>
          </view>
        ...
        <view class="order-footer">
          <div class="order-item">
            <div class="label">来源单据:</div>
            {{ order.sourceOrder }}
          </div>
          <div class="order-item">
            <div class="label">来源类型:</div>
            {{ order.sourceType_dictText }}
          </div>
          <div class="order-item">
            <div class="label">结息日：</div>
            {{ order.interestEndDate }}
          </div>
          <div class="order-item">
...
const getOrderList = async (type = 'more') => {
  loading.value = true
  try {
    const res = await getAction('/fms/redeemGoodsOrder/list', {
      pageNo: pageNo.value,
...
const viewOrderDetail = (order) => {
  uni.navigateTo({
    url: `/pages/finance/redeem-detail-info?id=${order.id}`,
  })
}
