...
import { getAction, putAction, postAction } from '@/common/store/manage'
...
const getAddressDetail = async (id) => {
  getAction(`/catalog/customerAddr/queryById`, {
    id: id,
  }).then((res) => {
    if (res.success) {
      ...
      // 设置国家选择器的索引
      const index = countryList.value.findIndex((item) => item.code === data.country)
      if (index > -1) {
...
const handleSubmit = () => {
  ...
  const action = addressId.value ? putAction : postAction
  const url = addressId.value ? '/catalog/customerAddr/edit' : '/catalog/customerAddr/add'

  action(url, formData.value).then((res) => {
    if (res.success) {
      uni.showToast({
        title: '保存成功',
        icon: 'success',
